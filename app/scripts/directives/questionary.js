...
app.run(['$templateCache', function($templateCache){
  ...
  $templateCache.put('radio-input.html','<div class="question--option" ng-repeat="opt in body.options"><input type="radio" id="radio--job-or-business-{{ idx }}-{{ $index }}" name="radio--job-or-business-{{ outerIdx }}-{{ idx }}" value="{{opt.value}}" ng-model="body.selected_value"><label for="radio--job-or-business-{{ idx }}-{{ $index }}">{{opt.label}}</label></div>');
